/*
 * \brief Get light intensity value (Lux & FootCandles) from Light dependent Resistor (LDR) a.k.a. photocell or
 * photoresistor
 *
...
public:
     ...
     * \param adc_resolution_bits (unsigned int, optional, default: 10) Number of resolution bits for the ADC pin (more
     * information here: https://www.arduino.cc/reference/en/language/functions/analog-io/analogread/)
     * \param smoothing_history_size (unsigned int, optional, default: 10) Max number of raw values used for \f
     * getSmoothedLux or \f getSmoothedFootCandles
     */
     ...
     *   3) Convert those 2 point into a "log linear curve" (with Excel for example): log(R[Ω]) = -0.6316 * log(I[lux])
     * + 4.7404 (linear) 4) Solve the equation to get I[lux]=mult_value/(R[Ω]^pow_value) approximation (with
     * wolframalpha for example): I[lux] ~= 32017200/R[Ω]^1.5832
    ...
    /*!
     * \brief getCurrentRawAnalogValue Read light intensity from the photocell, and provide the sensor raw analogic
     * value (not a really readable value)
     *
     * \return (int) Light intensity (in sensor raw analogic value), not a really readable value, prefer \f
     * getCurrentLux or \f getCurrentFootCandles
     */
     ...
     *
     * \param raw_value (int) Analog value of the photocell sensor (WARNING: This value must be with the same adc
     * resolution as the one in the constructor)
     *
     * \return (float) Light intensity (in lux)
     */
    ...
    /*!
     * \brief getSmoothedLux Read light intensity (in lux) from the photocell, apply linear smoothing using the number
     * of historic values specified with the constructor.
     *
    ...
    /*!
     * \brief getCurrentFootCandles Read light intensity from the photocell, apply linear smoothing using the number of
     * historic values specified with the constructor, convert to footcandles.
     *
...
private:
    ...
    float* _smoothing_history_values;      //!< (smoothing only) All valid values (in lux) in a table of \v
                                       //!< _smoothing_history_size values maximum (oldest value will be replaced by a
                                       //!< new one if table is full)
};
