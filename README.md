...
| Simple (dark)   |  | For dark rooms at night. The reading is drawn in one colour per glucose band, all white by default, so nothing lights up green; the trend arrow carries the band instead. Set the colours under Device settings, for example magenta in range and red for the urgent bands. Cyan means the reading is stale. No age bars. |
...
- Multiple clock faces support
  ...
  - Simple (dark) for the night: the reading in a calm colour, the glucose band on the trend arrow, per-band colours configurable
