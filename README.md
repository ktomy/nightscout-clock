...
| Time only       | | Only the time: hours, minutes and seconds in 24-hour format, or hours, minutes and AM/PM in 12-hour format. Glucose is hidden, so the clock can look like an ordinary clock, except that it shows the time and value like "Time and value" whenever the reading is urgent and while any glucose alarm sounds or is snoozed. With old or missing data only the time shows. |
...
- Multiple clock faces support
  ...
  - Time only, showing glucose only when the reading is urgent or a glucose alarm sounds or is snoozed; with old or missing data only the time shows
